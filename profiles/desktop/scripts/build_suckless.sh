...
install_slock() {
    ...
    if [ -d "$slock_dir" ] && [ -f "$slock_dir/config.h" ]; then
        ...
        # slock is NOT packaged in Debian trixie (apt fallback below always
        # fails) and the suckless submodule has no slock source — skip, don't
        # hard-fail the whole suckless install (set -e above).
        _process "slock: not in trixie repos and no source in submodule — skipping"
    fi
}

install_helpers() {
    _process "Desktop helpers (firefox alias)"
    # clipmenu is a declared source build now (opt/source/packages/clipmenu.conf,
    # built by ensure_source_pkgs on desktop profiles) — nothing ad-hoc here.
